...
## 当前状态
当前已经覆盖 `llmt_col`、矩阵级 dataset testbench、阵列顶层、MX 格式 helper、FP32 辅助模块、列级 testbench、阵列 smoke、Python 参考模型和脚本。后续如果继续扩展，按 [coverage_plan.md](coverage_plan.md) 做更细的逐行 Markdown 讲解或答辩问答。

如果目标是“逐行读代码”，不要继续把内容塞进本目录；请使用独立的 [逐行代码讲解目录](../line_by_line/README.md)。该目录只写 Markdown，不往 RTL、testbench 或脚本源文件里新增逐行注释。
