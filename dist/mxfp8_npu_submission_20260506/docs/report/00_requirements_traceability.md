...
| --- | --- | --- | --- |
| 书面报告：电路实现方案、验证方案、精度测试结果、综合结果 | `docs/report/submission_report.md`、`docs/report/01_problem_and_goals.md` 到 `docs/report/12_backend_handoff_checklist.md` | 可作为初赛报告正文 | `submission_report.md` 是第一入口；`03` 到 `07` 已按 RTL handoff 边界重写；`10` 到 `12` 汇总技术方案、打包边界和后端接收；`07_synthesis_and_ppa.md` 只给综合模板和 PPA 方法，真实 28nm 结果阻塞于库和工具 |
| 仿真验证报告：含仿真波形图等 | `docs/report/05_verification_methodology.md`、`reports/verification/`、`reports/evidence/` | 日志、VCD 和 PNG 截图证据具备 | 2026-05-06 fast/release 验收均为 `PASS_WITH_EXTERNAL_SYNTH_BLOCKER`；默认回归、Python golden、波形 smoke 日志、VCD 和 PNG 截图已刷新；证据见 `reports/evidence/waveform_capture_status.md` |
...
| 测试验证报告 | `docs/report/05_verification_methodology.md`、`reports/verification/`、`reports/evidence/` | 可提交 | 2026-05-06 已刷新默认回归日志、Python golden 日志、小 VCD 与报告级 PNG；后端门级验证仍等待真实综合环境 |
...
| RTL code 功能正确 | `rtl/`、`tb/`、`sim/run_iverilog.ps1`、`sim/run_python_ref.ps1` | 已复验 | 2026-05-06 默认 Verilog 回归与 Python golden 均已刷新到 `reports/verification/*.log` |
...
| 报告逻辑清晰 | `docs/report/README.md`、`docs/report/submission_report.md`、`docs/report/10_technical_solution_and_execution_plan.md`、`docs/admin/final_submission_manifest.md` | 基线具备 | 章节顺序、术语边界、总方案入口、提交边界和证据引用已统一；最终格式等待主办方模板 |
| 创新性/专项突出 | `docs/report/08_optimization_and_finals_outlook.md`、`docs/report/submission_report.md` | 基线具备 | 突出 MXFP8、非有限值语义、可复验向量和后端 handoff 边界；教学资料保留在仓库中，但不进入第一轮正式包 |
